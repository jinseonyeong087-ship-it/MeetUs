...
AI Minutes 프로젝트에서 AA 파트가 수행한 화면 설계, 사용자 흐름 정리, API 연동 문서화, AWS 프론트 배포 구조 정리, 배포 증적 정리를 모아둔 문서다.

이 문서는 프론트 구현 상세 자체보다 클라우드 사용 경험, 배포 구조 이해, 환경 구분, 문서 정합성 확보에 초점을 둔다.
