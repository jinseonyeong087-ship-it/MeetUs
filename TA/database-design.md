...
| password_hash | VARCHAR(255) | NOT NULL         | 사용자 비밀번호 (bcrypt hash) |
...
| audio_s3_key | TEXT         | NULL       | S3 음성 파일 경로 |
| created_at   | TIMESTAMP    | NOT NULL   | 생성 시간       |
...
| assignee_member_id | UUID      | FK, NULL   | 담당 멤버       |
...
AI 결과 payload의 assignee 문자열은 다음 규칙으로 매핑된다.

```
AI assignee (string)
↓
workspace_members.user.name 조회
↓
member_id 매핑
```
